...
import static ru.netology.Homework36.httpServer.Main.GET;
import static ru.netology.Homework36.httpServer.Main.POST;
...
public class Server {
    ...
    public void handle(Socket socket) {
        try (final var in = new BufferedInputStream(socket.getInputStream());
            ...
            //получаем отдельно путь без параметров и параметры
            String queryFromPath = null;
            if (path.contains("?")) {
                System.out.println("path with query: " + path);
                String[] newPath = path.split("\\?");
                path = newPath[0];
                queryFromPath = newPath[1];
            }
            ...
            final var request = new Request(method, headers.toString(), body, path, queryFromPath);
